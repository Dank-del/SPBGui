...
namespace SPB.GameObjects.ServerObjects
{
    public static class ThereIsServer
    {
        public struct Actions
        {
                    ...
                    if (ThereIsConstants.Forming.TheMainForm.ShowingSandBox is CreateProfileSandBox mySandBox)
                    {
                        ...
                        else if (mySandBox.IsCreatingEnded)
                        {
                            ...
                            else if (ServerSettings.MyProfile.IsSecuredMeWorkingOver)
                            {
                                ...
                                ThereIsConstants.AppSettings.MainClient = new MainClient(true);
                                ThereIsConstants.AppSettings.MainClient.Show();
                                ThereIsConstants.AppSettings.MainClient.FirstTimeDesigning();
                                await Task.Run(() =>
                                {
                ...
                if (ThereIsConstants.Forming.TheMainForm.IsShowingSandBox)
                {
                    if (ThereIsConstants.Forming.TheMainForm.ShowingSandBox is CreateProfileSandBox mySandBox)
                    {
                        ...
                        else if (mySandBox.IsSignInEnded)
                        {
                            ...
                            mySandBox.LoadingSandBox.Close(true);
                            mySandBox.Close(true);
                            ThereIsConstants.AppSettings.MainClient = new MainClient(false);
                            ThereIsConstants.AppSettings.MainClient.Show();
                            await Task.Run(() =>
                            {
            ...
            /// <summary>
            /// Worker for <see cref="MainClient.AnimationFactory"/>.
            /// </summary>
            /// <param name="sender"></param>
            /// <param name="e"></param>
            public static void TimeWorkerWorksForMainClientLoading(object sender, EventArgs e)
            {
#if SERVER

                if (ThereIsConstants.Forming.MainClient.IsLoading && 
                    !ThereIsConstants.Forming.MainClient.IsLoadingEnded)
                {
                    if(ThereIsConstants.Forming.MainClient.MessageLabel1.Text.IndexOf("...") != -1)
                    {
                        ThereIsConstants.Forming.MainClient.MessageLabel1.SetLabelText();
                    }
                    else
                    {
                        ThereIsConstants.Forming.MainClient.MessageLabel1.Text += ".";
                    }
                }
                else if (ThereIsConstants.Forming.MainClient.IsLoadingEnded)
                {
                    ((Timer)sender).Enabled = false;
                    ((Timer)sender).Dispose();
                    if(GameObjects.MyProfile.PlayerKingdom != SPB_Kingdoms.NotSet)
                    {
                        GameObjects.MyProfile.KingdomInfo =
                        await KingdomInfo.GetKingdomInfo((uint)GameObjects.MyProfile.PlayerKingdom);
                    }
                    ThereIsConstants.Forming.MainClient.AnimationFactory = null;
                    ThereIsConstants.Forming.MainClient.MainClientHandler();
                    GC.Collect();
                }
#endif
            }
        ...
        public struct ServerSettings
        {
            ...
            internal const string ServerChecking_File_Name = "Status.SPB";
            /// <summary>
